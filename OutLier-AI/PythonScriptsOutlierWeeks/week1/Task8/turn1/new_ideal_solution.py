...
def validate_signals(signals, required_mics=NUM_MICS):
...
def normalize_to_int24(signal: np.ndarray) -> np.ndarray:
    """
...
def validate_inputs(target_signal: np.ndarray, noise_signal: np.ndarray, doa_angles: List[float]) -> None:
    """
...
def simulate_microphone_signals(target_signal: np.ndarray, 
    ...
    validate_inputs(target_signal, noise_signal, doa_angles)
    ...
    try:
        for i in range(NUM_MICS):
            ...
            for frame_start in range(0, len(target_signal), FRAME_SIZE):
                ...
                combined_frame = normalize_to_int24(frame_signal + frame_noise)
...
def estimate_doa(signals: np.ndarray) -> float:
    """
...
def delay_and_sum(signals: np.ndarray, doa_angle: float) -> np.ndarray:
    """Simple delay-and-sum beamformer."""
    validate_signals(signals)
    validate_angle(doa_angle)
...
if __name__ == "__main__":
    try:
        ...
        # Convert to int24 precision
        target_signal = normalize_to_int24(target_signal)
        noise_signal = normalize_to_int24(noise_signal)
