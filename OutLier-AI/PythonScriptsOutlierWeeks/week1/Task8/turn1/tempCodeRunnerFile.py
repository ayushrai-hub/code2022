...
def simulate_microphone_signals(target_signal, noise_signal, doa_angles):
    ...
    for i in range(NUM_MICS):
        ...
        mic_signals[i, :] += np.roll(target_signal, delay_samples)
        mic_signals[i, :] += noise_signal
    return mic_signals
...
def delay_and_sum(signals, doa_angles):
    """
    ...
    for mic_idx in range(NUM_MICS):
        ...
        delay = (MIC_DISTANCE * mic_idx * np.cos(np.deg2rad(doa_angles[mic_idx]))) / SOUND_SPEED
        delay_samples = int(delay * SAMPLE_RATE)
...
if __name__ == "__main__":
    ...
    beamformed_signal = delay_and_sum(mic_signals, doa_angles)
