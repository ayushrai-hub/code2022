...
class MasteringEngine:
    def __init__(self, similarity_threshold: int = 80, source_priority: List[str] = None):
        self.validate_similarity_threshold(similarity_threshold)
        self.similarity_threshold = similarity_threshold
        self.source_priority = source_priority or ['Amazon', 'eBay']
        self.validate_source_priority(self.source_priority)
        self.matching_log = []
        self.logger = logging.getLogger(__name__)
        logging.basicConfig(level=logging.INFO)

    def validate_similarity_threshold(self, threshold: int):
    ...
    def validate_source_priority(self, priority: List[str]):
        valid_marketplaces = {'Amazon', 'eBay'}
        if not all(marketplace in valid_marketplaces for marketplace in priority):
